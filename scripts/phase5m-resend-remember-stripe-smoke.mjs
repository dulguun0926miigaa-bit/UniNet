...
has(schema, 'ticketTokenHash  String?            @unique', 'event QR token hash is persisted uniquely')
...
has(payments, 'hashEventTicket(createEventTicket', 'paid webhook persists only the event QR token hash')
...
has(studentRoutes, "registration.content.pricingType !== 'PAID'", 'QR ticket is restricted to paid events')
has(studentRoutes, "registration.payment?.status !== 'PAID'", 'QR ticket is blocked until payment is paid')
has(studentRoutes, 'ticketTokenHash: null', 'QR token hash is issued only once')
...
has(operations, 'where: { ticketTokenHash }', 'attendance finds only a DB-hashed QR token')
has(operations, "registration.payment?.status !== 'PAID'", 'attendance blocks every unpaid ticket')
...
has(studentUi, 'Stripe Sandbox', 'Student paid-event checkout is labeled sandbox')
