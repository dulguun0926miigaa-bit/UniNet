...
import UniversityLogo from "../dashboard/UniversityLogo";
import { normalizeUniversityName } from "../dashboard/universityBranding.js";
...
import { formatDate as formatPreferenceDate, formatDateTime } from "../settings/uiPreferences.js";
...
function formatDate(date = new Date()) {
  return formatPreferenceDate(date, { dateStyle: "full" });
}
  ...
  return (
    <>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        {group === "ACCOUNT" && <div className="space-y-4"><h2 className="font-display font-bold">Account</h2><label className="block text-xs font-bold">Timezone<NativeStyledSelect className="mt-2 block w-full rounded-lg border border-slate-200 p-3"><option>Asia/Ulaanbaatar</option></NativeStyledSelect></label></div>}
...
export default function StudentExperience({ user, onLogout, GlobalStyles }) {
  ...
  return (
    <div className="font-body min-h-screen bg-slate-50 text-slate-900">
      ...
      {flow === "TIMELINE" && <Modal title="Өргөдлийн timeline" onClose={() => setFlow(null)}><div className="space-y-0">{(selected.timeline?.length ? selected.timeline : [{ id: "current", status: selected.status, date: selected.appliedAt }]).map((entry, index) => <div key={entry.id || `${entry.status}-${index}`} className="flex gap-4 pb-6"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-600 text-xs font-bold text-white">{index + 1}</div><div><h3 className="text-sm font-bold">{entry.status}</h3><p className="mt-1 text-xs text-slate-400">{entry.date ? formatDateTime(entry.date) : "Огноо тодорхойгүй"}</p>{entry.reason && <p className="mt-1 text-xs text-slate-500">{entry.reason}</p>}</div></div>)}</div></Modal>}
