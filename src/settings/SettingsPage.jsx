...
export default function SettingsPage({ user, onLogout }) {
  ...
  const changeSection = section => setActive(section);
  const validate = () => {
    const errors = {};
    if (active === "account") {
    ...
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-[230px_1fr]">
        ...
        <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 md:p-7">
          ...
          {active === "notifications" && <section><h2 className="font-display text-xl font-bold">Мэдэгдэл</h2><div className="mt-5 grid gap-x-8 lg:grid-cols-2">{[["inApp", "In-app"], ["email", "Email"], ["push", "Push"], ["opportunities", "Шинэ боломж"], ["eventReminder", "Арга хэмжээний сануулга"], ["applicationStatus", "Өргөдлийн статус"], ["waitlist", "Waitlist шинэчлэлт"], ["surveyDeadline", "Судалгааны хугацаа"], ["announcements", "Зарлал"], ["system", "Системийн мэдэгдэл"]].map(([key, label]) => <Toggle key={key} label={label} checked={settings.notifications[key]} onChange={value => update("notifications", key, value)} />)}</div><label className={`${labelClass} mt-5 max-w-sm`}>Notification frequency<NativeStyledSelect value={settings.notifications.frequency} onChange={event => update("notifications", "frequency", event.target.value)} className={fieldClass}><option>Шууд</option><option>Өдөрт нэг удаа</option><option>7 хоногт нэг удаа</option></NativeStyledSelect></label></section>}
          {active === "privacy" && <section><h2 className="font-display text-xl font-bold">Нууцлал ба зөвшөөрөл</h2><div className="mt-5 grid gap-4 md:grid-cols-2"><label className={labelClass}>Profile visibility<NativeStyledSelect value={settings.privacy.profileVisibility} onChange={event => update("privacy", "profileVisibility", event.target.value)} className={fieldClass}><option>Миний сургууль</option><option>Зөвхөн би</option><option>UniNet сүлжээ</option></NativeStyledSelect></label><label className={labelClass}>CV sharing preference<NativeStyledSelect value={settings.privacy.cvSharing} onChange={event => update("privacy", "cvSharing", event.target.value)} className={fieldClass}><option>Зөвхөн зөвшөөрсөн өргөдөл</option><option>Хэзээ ч үгүй</option></NativeStyledSelect></label></div><Toggle label="Recommendation data preference" checked={settings.privacy.recommendations} onChange={value => update("privacy", "recommendations", value)} /><h3 className="font-display mb-3 mt-7 font-bold">Consent history</h3><div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="bg-slate-50"><tr><th className="p-3">Огноо</th><th>Хүлээн авагч</th><th>Зорилго</th><th>Хуваалцсан мэдээлэл</th></tr></thead><tbody>{settings.consentHistory.map(item => <tr key={item.id} className="border-t border-slate-100"><td className="p-3">{formatDate(item.grantedAt || item.date)}</td><td>{item.recipient}</td><td>{item.purpose}</td><td>{item.data}</td></tr>)}</tbody></table></div><button type="button" onClick={() => download("personal-data")} className="mt-5 rounded-lg border border-slate-200 px-4 py-2 text-xs font-bold">Personal data татах</button></section>}
          {active === "appearance" && <section><h2 className="font-display text-xl font-bold">Харагдах байдал</h2><div className="mt-5 grid grid-cols-3 gap-3">{["light", "dark", "system"].map(item => <button key={item} type="button" onClick={() => update("appearance", "theme", item)} className={`rounded-xl border p-5 text-xs font-bold ${settings.appearance.theme === item ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200"}`}>{item === "system" ? "System default" : item}</button>)}</div><label className={`${labelClass} mt-6`}>Density<NativeStyledSelect value={settings.appearance.density} onChange={event => update("appearance", "density", event.target.value)} className={fieldClass}><option value="compact">Compact</option><option value="comfortable">Comfortable</option></NativeStyledSelect></label><Toggle label="Reduced motion" checked={settings.appearance.reducedMotion} onChange={value => update("appearance", "reducedMotion", value)} /></section>}
          {active === "locale" && <section><h2 className="font-display text-xl font-bold">Хэл ба бүс</h2><div className="mt-5 grid gap-4 md:grid-cols-2"><label className={labelClass}>Timezone<NativeStyledSelect value={settings.locale.timezone} onChange={event => update("locale", "timezone", event.target.value)} className={fieldClass}><option>Asia/Ulaanbaatar</option><option>UTC</option></NativeStyledSelect></label><label className={labelClass}>Date format<NativeStyledSelect value={settings.locale.dateFormat} onChange={event => update("locale", "dateFormat", event.target.value)} className={fieldClass}><option>YYYY.MM.DD</option><option>DD/MM/YYYY</option></NativeStyledSelect></label><label className={labelClass}>Hour format<NativeStyledSelect value={settings.locale.hourFormat} onChange={event => update("locale", "hourFormat", event.target.value)} className={fieldClass}><option value="24">24 цаг</option><option value="12">12 цаг</option></NativeStyledSelect></label></div></section>}
          {active === "accessibility" && <section><h2 className="font-display text-xl font-bold">Хүртээмж</h2><label className={`${labelClass} mt-5`}>Font size<NativeStyledSelect value={settings.accessibility.fontSize} onChange={event => update("accessibility", "fontSize", event.target.value)} className={fieldClass}><option value="small">Жижиг</option><option value="normal">Хэвийн</option><option value="large">Том</option></NativeStyledSelect></label>{[["highContrast", "High contrast"], ["reducedMotion", "Reduced motion"], ["focusIndicator", "Visible focus indicator"], ["underlineLinks", "Underline links"]].map(([key, label]) => <Toggle key={key} label={label} checked={settings.accessibility[key]} onChange={value => update("accessibility", key, value)} />)}</section>}
