...
export default function SettingsPage({ user, onLogout }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const result = await settingsService.get(user); setSettings(result); setInitial(structuredClone(result)); }
  ...
  const save = async () => {
    ...
    try {
      ...
      await settingsService.save(active, active === "security" ? { ...passwords, twoFactor: settings.security.twoFactor } : settings[active], stepUpToken);
  ...
  const cancel = () => { setSettings(structuredClone(initial)); setPasswords({ current: "", next: "", repeat: "" }); setInlineErrors({}); };
        ...
        <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 md:p-7">
          ...
          {active === "security" && <section><h2 className="font-display text-xl font-bold">Аюулгүй байдал</h2><div className="mt-5 max-w-xl space-y-4"><label className={labelClass}>Одоогийн нууц үг *<input type="password" value={passwords.current} onChange={event => setPasswords(value => ({ ...value, current: event.target.value }))} className={fieldClass} />{inlineErrors.current && <span className="text-xs text-rose-600">{inlineErrors.current}</span>}</label><label className={labelClass}>Шинэ нууц үг *<input type="password" value={passwords.next} onChange={event => setPasswords(value => ({ ...value, next: event.target.value }))} className={fieldClass} /><div className="mt-2 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-blue-600 transition-all" style={{ width: `${passwordStrength}%` }} /></div>{inlineErrors.next && <span className="text-xs text-rose-600">{inlineErrors.next}</span>}</label><label className={labelClass}>Нууц үг давтах *<input type="password" value={passwords.repeat} onChange={event => setPasswords(value => ({ ...value, repeat: event.target.value }))} className={fieldClass} />{inlineErrors.repeat && <span className="text-xs text-rose-600">{inlineErrors.repeat}</span>}</label><div className="rounded-2xl border border-blue-100 bg-blue-50/40 p-4">
            {isAdminMfaRole ? <>
              ...
              {settings.security.mfaEnabled && <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className={labelClass}>Одоогийн нууц үг<input type="password" value={mfaPassword} onChange={event => setMfaPassword(event.target.value)} className={fieldClass} /></label><label className={labelClass}>Authenticator / recovery code<input value={mfaCode} onChange={event => setMfaCode(event.target.value)} className={fieldClass} /></label><button type="button" onClick={regenerateRecoveryCodes} className="rounded-lg border border-blue-200 px-4 py-2.5 text-xs font-bold text-blue-700">Recovery code шинэчлэх</button><p className="text-xs text-slate-500 sm:col-span-2">Үлдсэн recovery code: {settings.security.recoveryCodesRemaining ?? 0}</p></div>}
          ...
          {active === "help" && <section><h2 className="font-display text-xl font-bold">Help</h2><p className="mt-2 text-sm text-slate-500">UniNet ашиглахтай холбоотой түгээмэл асуулт болон тусламж.</p><div className="mt-6 space-y-3">{["Бүртгэл болон нэвтрэлт", "Арга хэмжээнд бүртгүүлэх", "Өргөдлийн төлөв хянах", "Нууцлал ба аюулгүй байдал"].map(item => <details key={item} className="rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer text-sm font-bold">{item}</summary><p className="mt-3 text-sm leading-relaxed text-slate-500">Энэ хэсгийн дэлгэрэнгүй заавар болон шийдлүүдийг эндээс харна.</p></details>)}</div><a href="mailto:support@uninet.mn" className="mt-6 inline-flex rounded-lg bg-slate-900 px-5 py-3 text-xs font-bold text-white">Support-той холбогдох</a></section>}
          {active === "feedback" && <section><h2 className="font-display text-xl font-bold">Send feedback</h2><p className="mt-2 text-sm text-slate-500">Санал хүсэлт, асуудал эсвэл шинэ боломжийн санаагаа илгээнэ үү.</p><form className="mt-6 max-w-2xl space-y-4" onSubmit={sendFeedback}><label className={labelClass}>Төрөл<select name="type" required className={fieldClass}><option>Санал хүсэлт</option><option>Алдаа мэдээлэх</option><option>Шинэ боломж санал болгох</option></select></label><label className={labelClass}>Гарчиг<input name="title" required className={fieldClass} /></label><label className={labelClass}>Дэлгэрэнгүй<textarea name="details" required rows="7" className={fieldClass} /></label><button disabled={saving} className="rounded-lg bg-slate-900 px-5 py-3 text-xs font-bold text-white disabled:opacity-60">{saving ? "Илгээж байна..." : "Илгээх"}</button></form></section>}
