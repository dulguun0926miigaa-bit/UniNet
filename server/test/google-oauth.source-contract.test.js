...
describe('Google OAuth account ownership source contract', () => {
  it('uses issuer plus provider subject and gates email prelinks through an explicit allowlist', () => {
    ...
    expect(service).toContain('resolveLegacyGoogleAccountPrelink(identity.gmail)')
    expect(service).toContain("audit('GOOGLE_PRELINK_LEGACY_STAFF_EMAIL_MIGRATED'")
    expect(service).toContain("audit('GOOGLE_PRELINK_STAFF_PROVISIONED'")
    expect(service).toContain("role: 'STAFF'")
